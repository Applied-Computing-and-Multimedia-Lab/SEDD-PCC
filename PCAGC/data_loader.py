...
from data_utils import read_h5_geo, read_h5_attri, read_ply_ascii_geo, read_ply_ascii_attri
...
class PCDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if idx in self.cache:
            ...
            # attri = features/255
            # feats = np.expand_dims(np.ones(coords.shape[0]) * 255, 1).astype('int')
            attri = features
            # new_feats = features
        ...
        # new_feats = add_gaussian_noise_to_point_cloud(new_feats)
...
if __name__ == "__main__":
    ...
    filedirs = sorted(glob.glob('./dataset'+'*.ply'))
