...
def huber_loss(ori_pts, dist_pts, delta):
    ori_colors = ori_pts.F
    dist_colors = dist_pts.F
    # dist_colors, ori_colors = rgb2yuv(bwd_colors.cpu(), ori_col.cpu())
    y_huber = torch.nn.functional.huber_loss(ori_colors[:, 0], dist_colors[:, 0], delta=delta)
    u_huber = torch.nn.functional.huber_loss(ori_colors[:, 1], dist_colors[:, 1], delta=delta)
    v_huber = torch.nn.functional.huber_loss(ori_colors[:, 2], dist_colors[:, 2], delta=delta)
    huber = torch.stack([y_huber, u_huber, v_huber], dim=0)
    return huber


def yuv_huber(ori_pts, dist_pts, delta):
    ...
    dist_colors = fwd_colors.cpu()
    ori_colors = ori_col.cpu()
    # dist_colors, ori_colors = rgb2yuv(dist_colors, ori_colors)
    y_huber = torch.nn.functional.huber_loss(ori_colors[:, 0], dist_colors[:, 0], delta=delta)
    u_huber = torch.nn.functional.huber_loss(ori_colors[:, 1], dist_colors[:, 1], delta=delta)
    v_huber = torch.nn.functional.huber_loss(ori_colors[:, 2], dist_colors[:, 2], delta=delta)
    huber_f = torch.stack([y_huber, u_huber, v_huber], dim=0)
    ...
    u_huber = torch.nn.functional.huber_loss(ori_colors[:, 1], dist_colors[:, 1], delta=delta)
    v_huber = torch.nn.functional.huber_loss(ori_colors[:, 2], dist_colors[:, 2], delta=delta)
    huber_b = torch.stack([y_huber, u_huber, v_huber], dim=0)

    huber = (huber_f + huber_b) / 2
    return huber